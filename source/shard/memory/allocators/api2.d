module shard.memory.allocators.api2;
    ...
    * If `memory` is not empty, and `new_length` is 0, the array will be
    ...
    Params:
        ...
        name            = An identifier used to enable tracking of the
                          ...
                          array.
    */
    bool reallocate_array(ref void[] memory, size_t element_size, length_t new_length, string name = no_name);
...
public @safe nothrow:
    ...
    object.
    */
    PtrType!T make(T, Args...)(Args args) {
        // max(object_size!T, 1) to support 0-size types (void, especially)
        void[] m = allocate(max(object_size!T, 1), fullyQualifiedName!T);
    ...
    */
    T[] make_array(T)(length_t length) {
        ...
        void[] m;
        if (!reallocate_array(m, T.sizeof, length, fullyQualifiedName!T))
            return null;
    ...
    */
    bool resize_array(T)(ref T[] array, length_t new_length) {
        ...
        bool do_resize() {
            void[] array_ = array;
            if (!reallocate_array(array_, T.sizeof, new_length, fullyQualifiedName!T))
                return false;
